    ...
    $(function(){
    	...
    	$('.Dbtn').on('click', function(){
    		...
    		if(check == true){
	    		...
	    		location.href = "<%=request.getContextPath()%>/jsp/DeleteMyRecipe.do?dish_code="+vdelete;
    		}else{
    			alert("삭제를 취소합니다.");
    		}
    	});
    	...
    	$(document).on('click', '.next', function(){
    		...
    		location.href="<%=request.getContextPath()%>/jsp/MyRecipe.do?cpage="+cc;
    	});
...
<!--                      <hr> -->
                    ...
                    <% if(startpage>1){ %>
                        <a href="<%=request.getContextPath()%>/jsp/MyRecipe.do?cpage=<%=currentpage-3%>"><i class="fa fa-long-arrow-left prev"></i></a>
                    <%} %>
                    ...
                    <%for(int i=startpage; i<=endpage; i++){
                    	if(currentpage==i){%>
                   		...
                   		<%}else{%>
	                       <a  href="<%=request.getContextPath()%>/jsp/MyRecipe.do?cpage=<%=i%>"><%=i%></a>
                    <%}} %>
                        </div>
                        
                    <%if(endpage < totalpage){ %>    
                        <a href="<%=request.getContextPath()%>/jsp/MyRecipe.do?cpage=<%=currentpage+3%>"><i class="fa fa-long-arrow-right next"></i></a>
                    <%} %>
                    </div>
                </div>
            </div>
